...
# כמות + שם-עצם מונה: "5 אנשים", "שלושה אנשים", "שני מוזמנים", "4 נפשות".
# המספר הוא `_NUM_TOKEN_RE` — כלומר **גם ספרות וגם מילות-מספר בעברית**. זה
# מכוון: כשהמספר הוא מילה ("שלושה אנשים"), חייבים לתפוס את הביטוי **כולו**,
# אחרת מילת-המספר נתפסת לבדה ושם-העצם ("אנשים") נשאר ונדבק לשם המוזמן.
_COUNT_WORD_RE = re.compile(
    rf"(?P<n>{_NUM_TOKEN_RE})\s*(?:אנשים|מוזמנים|נפשות|איש|אורחים)"
)
...
def _parse_quantity(working: str) -> tuple[Optional[str], Optional[int], str]:
    ...
    for rx in (_COUNT_WORD_RE, _COUNT_PAREN_RE, _COUNT_X_RE):
        ...
        if m:
            text = m.group(0).strip()
            rest = working[: m.start()] + " " + working[m.end():]
            # `_num_token_value` ולא `int()` — כי `_COUNT_WORD_RE` יכול לתפוס
            # מילת-מספר עברית ("שלושה אנשים"). לספרות הוא מתנהג כמו int().
            return text, _num_token_value(m.group(1)), rest
...
# ---------------------------------------------------------------------------
# נרמול שורה — שלב חובה לפני כל זיהוי
# ---------------------------------------------------------------------------
# תווי כיווניות ותווים בלתי-נראים (Unicode category Cf): RLM/LRM/RLE/PDF/
# isolates/BOM/zero-width. **זה לא מקרה קצה תיאורטי** — הדבקה אמיתית של
# רשימה עברית מ-WhatsApp, ממקלדת עברית בנייד או מאקסל ב-RTL מזריקה סימני
# כיווניות **דווקא סביב מספרים**, כי זה בדיוק מה ששולט על סדר התצוגה שלהם
# בתוך טקסט עברי. התוצאה: שורה שנראית למשתמש "תומר אדרי 0533333333 2"
# מכילה בפועל "2‏" בסוף.
#
# למה זה שבר את הפענוח: עוגני הסוף בביטויים הרגולריים ("\s*$", "(?:\s|$)")
# בודקים רווח-או-סוף-מחרוזת, ו-RLM הוא **לא** רווח (קטגוריה Cf) ולא סוף —
# ולכן הכמות פשוט לא נתפסה, ונפלה בשלמותה לתוך שדה השם. זה המקור המדויק
# לתלונה "המספרים והמילים 'זוג'/'שלושה' נכנסים לשם".
_INVISIBLE_MARKS_RE = re.compile(
    r"[­​-‏‪-‮⁠-⁤⁦-⁩﻿]"
)
# רווחים "אקזוטיים" (NBSP, narrow NBSP, en/em spaces, ideographic) — מנורמלים
# לרווח רגיל כדי שכל המפרידים בהמשך יהיו אחידים וצפויים.
_UNICODE_SPACE_RE = re.compile(r"[   -   　\t]+")


def _normalize_line(raw: str) -> str:
    """מנקה שורה גולמית **פעם אחת**, לפני שרץ עליה ולו ביטוי רגולרי אחד.

    זו נקודת החנק היחידה של הפענוח: מכאן והלאה כל שאר השלבים (טלפון, כמות,
    שם, רמז משפחה) עובדים על טקסט "נקי" — בלי תווים בלתי-נראים ובלי רווחים
    חריגים. הגישה הזו מכוונת: התיקון הוא בשלב אחד מרכזי ולא הוספת תו חריג
    לכל ביטוי רגולרי בנפרד, כך שגם ביטוי שיתווסף בעתיד מוגן אוטומטית.
    """
    text = _INVISIBLE_MARKS_RE.sub("", raw)
...
def parse_line(line: str) -> dict:
    ...
    """
    # 0) נרמול — חובה לפני כל זיהוי. מסיר תווי כיווניות בלתי-נראים שמגיעים
    # בהדבקה אמיתית מ-WhatsApp/נייד/אקסל ושוברים את עוגני הסוף של הביטויים
    # הרגולריים (ראו `_normalize_line`). כל השלבים מכאן עובדים על `line` הנקי.
