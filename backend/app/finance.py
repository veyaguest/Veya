...
def payments_total(expense: models.EventExpense) -> int:
    """סכום יומן התשלומים של השורה — **בלי חיתוך.**

    זה המספר שמוצג כ"שולם עד עכשיו" בפירוט התשלומים, והוא חייב להישאר
    נאמן למה שהזוג באמת רשם. החיתוך לעלות השורה קורה רק ב-
    ``paid_for_line``, ששם הוא נחוץ כדי ש"נשאר לשלם" לא ייצא שלילי.
    """
    return sum(max(p.amount_agorot or 0, 0) for p in (expense.payments or []))
...
def paid_for_line(expense: models.EventExpense, line_total_agorot: int) -> int:
    ...
    **נקודת האמת היחידה לשאלה "כמה כבר שילמנו".** הכותרת, כותרת הקבוצה,
    הסיכום והדוח קוראים כולם מכאן — ארבעה מקומות שסופרים תשלומים בארבע
    דרכים הם ארבעה מספרים שיסטו זה מזה מול חשבונית.

    **המקור הוא יומן התשלומים** (``ExpensePayment``): עלות השורה פחות סכום
    התשלומים היא "נשאר לשלם", בדיוק כפי שזוג מחשב את זה על מקרר.

    השדות הישנים (``is_paid`` / ``paid_amount_agorot``) הם **מסלול נפילה
    לשורות שנוצרו לפני היומן ועדיין לא הומרו**. ההמרה רצה פעם אחת בעליית
    השרת (``main._migrate_expense_payments``), ואחריה הענף הזה לא נדרש —
    אבל הוא נשאר, כי שרת שעולה על DB ישן חייב להציג מספר נכון גם ברגע
    שלפני ההמרה.

    הסכום **נחתך לעלות השורה**: זוג ששילם 20,000 ומאוחר יותר הוריד את
    המחיר ל-15,000 לא "שילם 20,000 מתוך 15,000". העודף הוא החזר, לא
    הוצאה, והוא לא שייך לצד ההוצאות — ובלי החיתוך "נשאר לשלם" היה יורד
    למספר שלילי בכותרת המסך.
    """
    ledger = payments_total(expense)
    if ledger:
        return min(ledger, line_total_agorot)

    # ── מסלול נפילה: שורה שקדמה ליומן ────────────────────────────────
    if expense.is_paid:
