...
# ── gifts (עסקאות מתנה) ──────────────────────────────────────────────────
# קריאה בלבד. במכוון **ללא** send_messages: מפיק ששולח הודעות למוזמנים לא
# אמור לראות בזכות זה גם כספים. כתיבה לא מופיעה כאן כי אין לה מדיניות
# בכלל — שינוי שורת כסף עובר רק דרך פונקציות SECURITY DEFINER מבוקרות
# (ראו rls/13_gifts_rls.sql).
GIFTS_VIEW = ["view_reports", "view_event"]
