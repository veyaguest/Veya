...
_LEGACY_BEFORE_EVENT_BODIES: list[tuple[str | None, str]] = [
    ...
    # גרסה ותיקה יותר (מלפני "[שם פרטי]"), שנמצאה בפועל בבדיקה מול נתוני
    # הייצור — כל 5 אירועי החתונה הקיימים החזיקו את הנוסח הזה בדיוק, ולכן
    # לא תואמים לאף אחד מהנוסחים החדשים יותר למעלה.
    (None, (
        "היי [שם אורח]! 🥂\n"
