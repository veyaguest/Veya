    ...
    for idx, tnum in enumerate(sorted(all_table_numbers)):
        ...
        out_tables.append(
            schemas.HallTable(
                ...
                width=float(pos["width"]) if pos.get("width") else None,
                height=float(pos["height"]) if pos.get("height") else None,
...
def save_hall(
    ...
    if payload.sketch is not None:
        event.hall_sketch = media.resolve_incoming(
            db, payload.sketch, event.hall_sketch, prefix=f"sketch-{event.id}"
        )
    db.commit()
