...
export interface HallGuest {
  ...
  rsvp_status: RsvpStatus
  is_child: boolean
  // הערות שרלוונטיות להושבה — חשיפה בלבד מהשרת, כדי שסרגל המוזמנים
  // שלצד הסקיצה יוכל להציג אותן בלי קריאת רשת נוספת.
  // ``notes_raw`` (ההערה הפנימית של הבעלים) **אינו** מגיע לכאן בכוונה:
  // מנוע ההושבה לא קורא אותו, ולכן גם מסך ההושבה לא מציג אותו.
  seating_notes?: string | null // מה שהבעלים כתב כהערת הושבה
  guest_note?: string | null // מה שהמוזמן עצמו מסר ב-RSVP
}
...
export interface HallState {
  ...
  // האם מדריך ההדרכה כבר נסגר פעם אחת באירוע הזה (ראו models.Event
  // בבקאנד). פעם אחת לכל אירוע, לא לכל דפדפן.
  guide_seen: boolean
}
...
export interface User {
  ...
  // לכל חשבון (ראו GuestsPage.tsx).
  guests_popup_seen?: boolean
}
