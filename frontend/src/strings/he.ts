...
export const strings = {
  ...
  messages: {
    ...
    statusCard: {
      ...
      subtitle: 'אנחנו עוקבים אחרי ההודעות שלכם ומעדכנים אתכם מה קרה לכל הודעה.',
      loadError: 'לא הצלחנו לטעון את מצב ההודעות כרגע. ננסה שוב',
      sent: 'נשלחו',
      sentHint: 'ההודעה הועברה בהצלחה לשליחה — עדיין אין אישור שהיא הגיעה למכשיר.',
      delivered: 'נמסרו',
      deliveredHint: 'וואטסאפ אישרה שההודעה נמסרה למכשיר של המוזמן.',
      read: 'נקראו',
      readHint: 'וואטסאפ אישרה שהמוזמן פתח וקרא את ההודעה, כשהמידע הזה זמין.',
      failed: 'לא נשלחו',
      failedHint: 'שליחת ההודעה נכשלה.',
      // "מספר לא תקין" — ידע מקומי בלבד (חסר/פורמט שגוי/לא ניתן לשימוש
      // לשליחה), לא בדיקה מול WhatsApp. אסור לכתוב "לא זמין ב-WhatsApp" —
      // זה היה מרמז שהמערכת קיבלה אישור מהספק, וזה לא המצב.
      noValidNumber: 'מספר לא תקין',
      noValidNumberHint: 'מספר הטלפון חסר, בפורמט לא תקין, או לא ניתן להשתמש בו לשליחה.',
      // "חסומים" — רק כשיש אינדיקציה אמיתית מהספק (לא ברירת מחדל על כל כשל).
      blocked: 'חסומים',
      blockedHint: 'השליחה נחסמה — יש אינדיקציה מהספק שהמוזמן לא רוצה לקבל הודעות.',
      queued: 'ממתינים לשליחה',
      queuedHint: 'יש מספר טלפון תקין, אבל עוד לא ניסינו לשלוח אליו הודעה.',
      readNote: 'סטטוס "נקראו" מגיע רק כשהמוזמן לא כיבה אישורי קריאה בוואטסאפ.',

      // בורר "מעקב אחר: X" — כל כרטיס מציג הודעה אחת בכל פעם, לא סיכום מצטבר.
      selectorLabel: 'מעקב אחר:',
      // מילות שורת הסיכום: "<כמות> מוזמנים · <כמות> נמסרו · <כמות> דורשים טיפול".
      guestsWord: 'מוזמנים',
      reachedWord: 'נמסרו',
      needsAttentionWord: 'דורשים טיפול',

      // מצב "עדיין לא נשלחה" — משפט שלם לכל סוג הודעה (לא תבנית אחת עם מגדר
      // משתנה: "תזכורת" נקבה, "יום האירוע" זכר — כתיבה ידנית מונעת טעות דקדוק).
      notSentYet: {
        invitation: 'עדיין לא נשלחה הזמנה.',
        reminder_1: 'עדיין לא נשלחה תזכורת ראשונה.',
        reminder_2: 'עדיין לא נשלחה תזכורת שנייה.',
        final_reminder: 'עדיין לא נשלחה תזכורת אחרונה.',
        event_day: 'עדיין לא נשלחה הודעת יום האירוע.',
        thank_you: 'עדיין לא נשלחה הודעת תודה.',
      } as Record<string, string>,
      notSentYetNote: 'ברגע שהיא תישלח, המעקב יופיע כאן.',

      // רשימת "מי קיבל את ההודעה" — חיפוש + סינון לפי סטטוס, נפרדת לחלוטין
      // מרשימת ה-RSVP (שנשארת במסך "תמונת מצב").
      guestListTitle: 'מי קיבל את ההודעה?',
  ...
  gifts: {
    ...
    // "3 מתנות" / "מתנה אחת" — נספח לסכום, ולכן בלי הפועל.
    giftsCount: (n: number) => (n === 1 ? 'מתנה אחת' : `${n} מתנות`),
    ...
    receivedCount: (n: number) => (n === 1 ? 'מתנה אחת התקבלה' : `${n} מתנות התקבלו`),
    ...
    emptyBody: 'כשמוזמן ישלח מתנה, היא תופיע כאן.',
