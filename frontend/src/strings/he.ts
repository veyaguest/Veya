...
export const strings = {
  ...
  messages: {
    ...
    when: {
      ...
      scheduled: (date: string) => `תצא ביום ${date}, בשעה שבחרתם למטה.`,
      sent: (date: string) => `יצאה ביום ${date}.`,
      waiting: (date: string) =>
      ...
      notInTrack:
        'לא נכנסת הפעם: נשארו פחות מ-14 ימים עד סגירת הרשימה, ולכן במסלול פחות סבבים.',
    },
    ...
    needsYou: {
      ...
      noAnswerDesc: 'הסבבים הסתיימו. אפשר לבדוק איתם ישירות, ולעדכן את התשובה ברשימה.',
      undecided: (n: number) =>
    ...
    trackPhase: {
      waitingTitle: 'מסלול אישורי ההגעה עוד לא התחיל',
      waitingBody:
        'בקשות אישור ההגעה יוצאות רק אחרי שתבחרו מועד סגירת רשימה, בעריכת פרטי האירוע בתמונת המצב. הלוח כאן הוא הערכה לפי יום לפני האירוע.',
      beforeTitle: (date: string) => `מסלול אישורי ההגעה מתחיל ב-${date}`,
      beforeBody:
        'עד אז לא יוצאת אף בקשת אישור הגעה. ההזמנה נשלחת בנפרד, מתי שתבחרו, והיא לא משנה את הלוח.',
      runningTitle: 'מסלול אישורי ההגעה בדרך',
      runningBody: 'VEYA שולחת את הסבבים לבד, לפי הלוח. אין צורך לעשות כלום כדי שהם ייצאו.',
      nowLabel: 'עכשיו',
      nextLabel: 'הסבב הבא',
      endedTitle: 'מסלול אישורי ההגעה הסתיים',
      endedBody: (date: string) =>
        `הרשימה נסגרה ב-${date}. התמונה למעלה היא המצב הסופי, ואפשר להתחיל בסידור ההושבה.`,
      endedCta: 'לסידור ההושבה',
      shortNote: 'נשאר פחות מ-14 ימים עד סגירת הרשימה, ולכן במסלול פחות סבבים.',
    },
