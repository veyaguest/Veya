...
/**
 * "9 מתנות התקבלו · 2 ממתינות · אחת הוחזרה" — שורה אחת מתחת לסכום.
 *
 * **רק מה ששייך לתמונת הכסף של הזוג.** קודם השורה מנתה גם מתנות
 * שנכשלו ושבוטלו — אבל אלה מעולם לא היו כסף אצל הזוג, ולכן אין להן מה
 * לחפש בשורה שמסבירה בדיוק את זה: כמה התקבל, כמה עוד בדרך, ואם משהו
 * שכבר נספר חזר. התוצאה הייתה נקראת כרשימת-סטטוסים טכנית ("דוח
 * עסקאות") במקום כתשובה על "כמה כסף יש לי". מתנה שנכשלה או בוטלה עדיין
 * מוצגת — עם התג השקט שלה, בשורה שלה, ברשימה המלאה מתחת (``logTitle``:
 * "כל המתנות", לא רק אלה שנספרו כאן).
 *
 ...
 */
const STATUS_ORDER = ['pending', 'refunded'] as const
