...
export function MessageBuilder({ invitationOnly = false }: { invitationOnly?: boolean } = {}) {
  ...
  const load = useCallback(async () => {
    try {
      const [tpls, ev, g] = await Promise.all([
        listAutomationTemplates(),
        getEvent(),
        listGuests('', 1, 0),
      ])
    ...
    <div className="mb-wrap" dir="rtl">
      ...
      {templates.length === 0 ? (
        ...
        <div className="mb-grid">
          ...
          {/* ---- כרטיס 2: עורך הנוסח — הספרייה היא דרך הבחירה המומלצת ---- */}
          <section className="mb-card mb-card-editor">
            <div className="mb-card-head">
              <h3 className="mb-card-title">{t.editorTitle}</h3>
            </div>

            <button type="button" className="mb-lib-cta" onClick={openLibrary}>
              <span className="mb-lib-cta-icon" aria-hidden="true">
                📚
              </span>
              <span className="mb-lib-cta-text">
                <span className="mb-lib-cta-title">
                  {selected && KIND_TO_CATEGORY[selected.kind]
                    ? t.libraryButtonFor(KIND_LABEL[selected.kind] ?? 'הודעה')
                    : t.libraryButton}
                </span>
                <span className="mb-lib-cta-sub">{t.libraryButtonHint}</span>
              </span>
              <span className="mb-lib-cta-arrow" aria-hidden="true">
                ‹
              </span>
            </button>
